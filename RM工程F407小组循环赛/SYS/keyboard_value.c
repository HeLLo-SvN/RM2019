...
int F_under_isle_flag=1;//岛下取前排弹药标志位(总决赛将F键位改为B)
...
//	}
    ...
    if(RC_Ctl.key.v&KEY_Q){    //陀螺仪控制180度的旋转
	...
	if(RC_Ctl.key.v&KEY_B){  //总决赛将F键位改为B
		
//	  if(E_under_isle_flag==1){
		F_under_isle_flag=0;
//	  }
		PID_SWITCH=0;//总决赛新增取弹模式PID的切换
